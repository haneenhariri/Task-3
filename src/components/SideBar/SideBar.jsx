    ...
    <nav className={`sidebar ${isOpen ? 'open' : ''}`}>
      <div>
        <div className="nav-head head-side">
          <h3>VILLA</h3>
          <Link to="" onClick={(e) => { e.preventDefault(); toggleSidebar(); }}>
            <img className="close" src="./imag/NavIcon/xmark-solid.svg" alt="close" />
          </Link>
        </div>
          ...
          {navData.map(index => {
            return (
              <li className='li-side' key={index.id}>
                <Link className='nav-link' to={index.link} onClick={toggleSidebar}>
                  {index.title}
                </Link>
              </li>
            );
      ...
      <div>
        <div className='div-1'>
          <div className='info info-side'>
            {infoData.map(i => {
              return (
                <Link to={i.href} key={i.id} className='info-link' onClick={toggleSidebar}>
          ...
          {infoIcon.map(j => {
            return (
              <div className='social-icon' key={j.id}>
                <Link to={j.href} className='link' onClick={toggleSidebar}>
